...
import { ChevronLeft, ChevronRight, SlidersHorizontal, MonitorPlay, CheckSquare, Plus, CalendarDays, Kanban, Maximize2, Minimize2, Check, Ban, Eye, LayoutList, AlignLeft, Circle, Package, Sparkles } from 'lucide-react';
            ...
            <div className="flex flex-col xl:flex-row items-center gap-5 xl:gap-8">
                ...
                <div className="flex flex-1 items-center gap-4 w-full xl:w-auto overflow-hidden">
                    ...
                    <div className="group flex items-center bg-white rounded-2xl shadow-sm border border-gray-100 shrink-0 h-12 p-1.5 hover:shadow-md hover:border-indigo-200 transition-all duration-300">
                        ...
                        <button 
                            ...
                            className="px-4 h-full flex items-center justify-center min-w-[160px] cursor-pointer hover:bg-slate-50 rounded-xl transition-all relative group/date select-none active:scale-95"
                    ...
                    <div className={`flex-1 overflow-hidden transition-all duration-500 ${showFilters ? 'opacity-100' : 'opacity-0 w-0'}`}>
                        <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide py-2 px-1 mask-fade-right">
                             <button
                                  ...
                                  className={`
                                      px-4 py-2 rounded-full text-xs font-bold transition-all duration-300 whitespace-nowrap shadow-sm border shrink-0 active:scale-95
                              ...
                              {visibleChips.map((chip) => {
                                  ...
                                  return (
                                      <button
                                          ...
                                          className={`
                                              px-4 py-2 rounded-full text-xs font-bold transition-all duration-300 whitespace-nowrap shadow-sm border shrink-0 flex items-center gap-1.5 active:scale-95
                                              ${baseClasses}
                                          `}
                                      >
                              ...
                              <button 
                                  ...
                                  className="w-9 h-9 flex items-center justify-center rounded-full transition-all bg-white hover:bg-indigo-50 text-gray-400 hover:text-indigo-600 border border-gray-200 hover:border-indigo-200 shrink-0 shadow-sm hover:shadow-md hover:rotate-90 active:scale-95" 
                                  title="จัดการตัวกรอง"
                              >
                ...
                <div className="flex items-center justify-between w-full xl:w-auto shrink-0 gap-3 border-t xl:border-t-0 border-gray-100 pt-4 xl:pt-0">
                    ...
                    <div className="relative" ref={viewMenuRef}>
                        ...
                        {isViewMenuOpen && (
                            <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 p-2 z-50 animate-in fade-in zoom-in-95 origin-top-right">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="relative bg-gray-100/80 p-1 rounded-xl flex items-center shrink-0 border border-gray-200/60 overflow-hidden w-[88px] h-10">
                            {/* Sliding Background */}
                            <div 
                                className={`absolute top-1 bottom-1 w-[38px] bg-white rounded-lg shadow-sm transition-all duration-300 ease-[cubic-bezier(0.25,1,0.5,1)] ${displayMode === 'CALENDAR' ? 'left-1' : 'left-[46px]'}`}
                            ></div>
                        ...
                        <div className="relative bg-gray-100/80 p-1 rounded-xl flex items-center shrink-0 border border-gray-200/60 overflow-hidden h-10 w-[180px]">
                             {/* Sliding Background */}
                             <div 
                                className={`absolute top-1 bottom-1 w-[86px] bg-white rounded-lg shadow-sm transition-all duration-300 ease-[cubic-bezier(0.25,1,0.5,1)] ${viewMode === 'CONTENT' ? 'left-1' : 'left-[90px]'}`}
                            ></div>

                            <button 
                                onClick={() => setViewMode('CONTENT')}
                                className={`
                                    relative z-10 flex-1 flex items-center justify-center gap-1.5 h-full rounded-lg text-xs font-bold transition-colors duration-300
                    ...
                    {/* Create Button (Gradient Pop with Smooth Transition) */}
                    <button 
                        onClick={() => onSelectDate(new Date(), viewMode)}
                        className={`
                            relative overflow-hidden group h-11 xl:h-auto px-3 xl:px-5 py-2.5 rounded-2xl text-white shadow-xl transition-all duration-500 active:scale-95 flex items-center justify-center shrink-0
                            ${viewMode === 'CONTENT' ? 'shadow-indigo-300/50' : 'shadow-emerald-300/50'}
                            hover:scale-105
                        `}
                    >
                        {/* Layered Gradients for Cross-Dissolve Effect */}
                        ...
                        <div className="relative z-10 flex items-center gap-2">
                            ...
                            <Sparkles className="w-3 h-3 text-yellow-300 absolute -top-1 -right-2 animate-pulse opacity-0 group-hover:opacity-100 transition-opacity" />
                        </div>
                    </button>
