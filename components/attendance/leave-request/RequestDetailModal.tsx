...
export const RequestDetailModal: React.FC<RequestDetailModalProps> = ({
    ...
    let durationText = '';
    if (isLeave) {
        const days = request.isHalfDay ? 0.5 : getWorkingDaysDifference(request.startDate, request.endDate, annualHolidays, calendarExceptions, null, true);
        const sessionSuffix = request.isHalfDay 
            ? ` (ครึ่งวัน${request.halfDaySession === 'AM' ? 'เช้า' : 'บ่าย'})`
            : '';
