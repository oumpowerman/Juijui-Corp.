...
export const ChannelCard: React.FC<ChannelCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`bg-white rounded-[2rem] border border-b-[3.5px] transition-all duration-300 group flex flex-col cursor-pointer relative hover:z-30 ${
        aura 
    ...
    >
      ...
      <div className={`h-28 w-full ${bgClass} relative rounded-t-[calc(2rem-2px)] overflow-hidden transition-all duration-500 ${aura?.bannerBorder || ''}`}>
        ...
        <div className="absolute top-3.5 right-4 flex items-center gap-1.5 z-10">
          {(channel.group_name || group?.name) && (
            <span 
              className="px-2.5 py-0.5 text-[9px] font-black rounded-full bg-white/85 backdrop-blur-md text-slate-800 border border-white/80 border-b-[1.5px] border-b-slate-200/60 shadow-2xs flex items-center gap-1 max-w-[130px] truncate"
      ...
      <div className="absolute top-14 left-6 z-20">
        {/* Floating Crown Badge for Top 10 */}
        {aura && (
