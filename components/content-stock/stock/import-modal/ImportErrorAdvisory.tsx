...
import { AlertTriangle, Lightbulb, Download, Filter, CheckCircle2, Sparkles, ArrowRight } from 'lucide-react';
import { ImportViewFilter } from './useImportPreviewModal';

interface ImportErrorAdvisoryProps {
...
export const ImportErrorAdvisory: React.FC<ImportErrorAdvisoryProps> = ({
    errorRowsCount,
    warningRowsCount,
    totalRows,
    onDownloadTemplate,
    ...
    // If no errors and no warnings, no banner needed
    if (errorRowsCount === 0 && warningRowsCount === 0) {
        return null;
    }

    // Case 1: Error rows > 5 -> Strongly advise fixing in CSV file first
    if (errorRowsCount > 5) {
        return (
                    ...
                    <div className="space-y-0.5">
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-full bg-rose-600 text-white tracking-wide">
                                แนะนำแก้ไขในไฟล์ CSV
                            </span>
                        </div>
                        <p className="text-[11px] text-slate-600 leading-relaxed font-medium">
                            เนื่องจากมีแถวที่ไม่ผ่านการตรวจสอบ<strong>มากกว่า 5 รายการ</strong> ขอแนะนำให้เปิดไฟล์ CSV ต้นฉบับแก้ไขในโปรแกรม Spreadsheet (เช่น เพิ่มชื่อ Topic หรือปรับชื่อช่อง) แล้วอัปโหลดใหม่อีกครั้งเพื่อความสะดวกรวดเร็ว — หรือหากต้องการ คุณสามารถกดปุ่ม <span className="font-bold text-slate-800">"แก้ไข"</span> ที่ตารางเพื่อแก้ไขเฉพาะบางแถวได้เช่นกัน
                        </p>
                    </div>
                </div>
    ...
    if (errorRowsCount > 0 && errorRowsCount <= 5) {
        return (
            <div className="mx-4 sm:mx-6 my-2.5 p-3.5 rounded-2xl bg-gradient-to-r from-amber-500/10 via-indigo-50/60 to-emerald-50/40 border border-amber-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 shadow-xs shrink-0">
                <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="space-y-0.5">
                        ...
                        <p className="text-[11px] text-slate-600 leading-relaxed font-medium">
                            คุณสามารถกดปุ่ม <span className="font-bold text-indigo-700">✏️ "แก้ไข"</span> ที่แถวในตารางเพื่อกรอกชื่อหัวข้อ หรือเลือกช่อง/ผู้รับผิดชอบให้ถูกต้อง เมื่อบันทึกแล้วแถวจะเปลี่ยนเป็น <span className="text-emerald-600 font-bold">"พร้อมลง"</span> ทันที
                        </p>
                    </div>
                </div>
    ...
    return (
        <div className="mx-4 sm:mx-6 my-2 p-2.5 sm:p-3 rounded-2xl bg-amber-50/60 border border-amber-200/70 flex items-center justify-between gap-3 shrink-0">
            <div className="flex items-center gap-2 text-xs text-amber-900 font-medium">
                ...
                <span>
                    ข้อมูลผ่านการตรวจสอบทั้งหมด มีเพียงคำเตือนการจับคู่ชื่อ {warningRowsCount} รายการ (ระบบจะตั้งค่าเริ่มต้นให้และสามารถนำเข้าได้ทันที)
                </span>
            </div>
            <button
                type="button"
            ...
            >
                {activeFilter === 'WARNING' ? 'แสดงทั้งหมด' : 'ดูคำเตือน'}
            </button>
        </div>
    );
};
