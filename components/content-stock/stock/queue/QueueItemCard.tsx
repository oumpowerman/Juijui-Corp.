...
const QueueItemCard: React.FC<QueueItemCardProps> = ({
    ...
    return (
        <motion.div
            ...
            className="relative"
        >
            {/* Sequence Number Badge - Popping Out */}
            <div className={`
            ...
            <button
                ...
                className={`
                    absolute top-3 right-3 z-30 w-6 h-6 rounded-xl border-2 flex items-center justify-center transition-all shadow-md
                    ${isSelected 
                        ? 'bg-indigo-600 border-indigo-600 text-white ring-2 ring-white scale-105' 
                        : 'bg-white/90 backdrop-blur-sm border-slate-300 text-transparent hover:border-indigo-500 hover:text-indigo-400'}
                `}
            ...
            <div
                ...
                className={`
                    ...
                    ${isSelected ? 'ring-2 ring-indigo-500 border-indigo-400 bg-indigo-50/15' : ''}
                    ${!isSelected && isContent 
                        ? 'border-amber-100 hover:border-amber-300 shadow-amber-50' 
                        : !isSelected ? 'border-blue-100 hover:border-blue-300 shadow-blue-50' : ''}
                    ${isFinished && !isSelected ? 'opacity-60 grayscale-[0.5]' : 'shadow-sm hover:shadow-md hover:-translate-y-1'}
                `}
            >
                {/* Type Indicator Bar */}
