...
import { Trophy, ChevronRight, Crown, TrendingUp } from 'lucide-react';
...
const HallOfFameWidget: React.FC<HallOfFameWidgetProps> = ({ users, currentUser, onNavigate }) => {
    ...
    // --- Logic: Rank Users ---
    const { top3, myRank, diffToNext } = useMemo(() => {
        // 1. Filter Active & Sort by XP
        const sorted = [...users]
            .filter(u => u.isActive)
            .sort((a, b) => b.xp - a.xp);
        ...
        // 3. Find Me
        const myIndex = sorted.findIndex(u => u.id === currentUser.id);
        const myRank = myIndex + 1;
        const myData = sorted[myIndex];

        // 4. Calculate diff to person above me
        let diffToNext = 0;
        if (myIndex > 0) {
            diffToNext = sorted[myIndex - 1].xp - myData.xp;
        }

        return { top3, myRank, myData, diffToNext };
    }, [users, currentUser]);

    // --- Components ---
    const RenderAvatar = ({ user, rank }: { user: User, rank: number }) => {
        ...
        return (
            <div className={`flex flex-col items-center ${mt} relative group`}>
                ...
                <span className="text-[9px] font-medium text-gray-400">{user.xp.toLocaleString()} XP</span>
            </div>
        );
    };
    ...
    return (
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-white/50 relative overflow-hidden flex flex-col h-full group hover:shadow-xl hover:shadow-orange-100 transition-all duration-500 min-h-[300px]">
                ...
                <div className="relative z-10 flex justify-between items-start">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-yellow-100 text-xs font-bold uppercase tracking-wider mt-1">Top Performers</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 p-2 flex items-center justify-center bg-gradient-to-b from-orange-50/30 to-white -mt-4 relative z-10 rounded-t-[2rem]">
                <div className="flex items-end justify-center gap-3 md:gap-6 pb-2 w-full">
                    {/* Rank 2 */}
                    {top3[1] ? <RenderAvatar user={top3[1]} rank={2} /> : <div className="w-12 h-20 flex items-end justify-center"><div className="w-10 h-10 rounded-full bg-gray-100"></div></div>}
                    
                    {/* Rank 1 */}
                    {top3[0] ? <RenderAvatar user={top3[0]} rank={1} /> : <div className="w-16 h-24 flex items-end justify-center"><div className="w-12 h-12 rounded-full bg-gray-100"></div></div>}
                    
                    {/* Rank 3 */}
                    {top3[2] ? <RenderAvatar user={top3[2]} rank={3} /> : <div className="w-12 h-20 flex items-end justify-center"><div className="w-10 h-10 rounded-full bg-gray-100"></div></div>}
                </div>
            </div>

            {/* My Rank Footer */}
            <div className="p-3 border-t border-gray-50 bg-gray-50/50">
                <div className="bg-white border border-gray-100 rounded-2xl p-3 flex items-center gap-3 shadow-sm hover:border-orange-200 transition-colors cursor-pointer group/rank" onClick={() => onNavigate('LEADERBOARD')}>
                    <div className="flex-1">
                        <div className="flex items-center gap-2 mb-0.5">
                            <span className="bg-slate-800 text-white text-[9px] font-black px-1.5 py-0.5 rounded shadow-sm">
                                #{myRank}
                            </span>
                            <span className="text-xs font-bold text-gray-700">อันดับของคุณ</span>
                        </div>
                        <p className="text-[10px] text-gray-500 flex items-center">
                            {myRank === 1 ? (
                                ...
                                <span className="flex items-center gap-1">
                                    ...
                                    ตามที่ {myRank - 1} อยู่ <b className="text-gray-700">{diffToNext.toLocaleString()}</b> XP
                                </span>
                            ) : (
