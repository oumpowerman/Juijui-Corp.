...
import { Camera, CheckCircle2, Loader2, Sparkles, ArrowRightLeft, HardDrive } from 'lucide-react';
    ...
    const handleFileClick = async () => {
        if (!isAuthenticated) {
            const connect = await showConfirm(
                'Google Drive ยังไม่ถูกเชื่อมต่อ',
                'การอัปโหลดจะใช้ระบบสำรอง (Supabase) ซึ่งมีพื้นที่จำกัด คุณต้องการเชื่อมต่อ Google Drive ก่อนอัปโหลดเพื่อให้ระบบสมบูรณ์ที่สุดหรือไม่?'
            );
            if (connect) {
                login();
                return;
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-violet-600 rounded-[2rem] p-6 text-white shadow-xl shadow-indigo-200 mb-6 relative overflow-hidden animate-in slide-in-from-top-4 duration-500">
            ...
            <div className="relative z-10 text-center mb-6">
                <div className="flex items-center justify-center gap-2 mb-3">
                    <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold border border-white/20">
                        <Sparkles className="w-3 h-3 text-yellow-300 animate-pulse" /> Daily Mission
                    </div>
                    <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black border transition-all duration-500 ${isAuthenticated ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' : 'bg-amber-500/20 text-amber-300 border-amber-500/30'}`}>
                        <HardDrive className={`w-3 h-3 ${isAuthenticated ? 'animate-pulse' : ''}`} />
                        {isAuthenticated ? 'DRIVE ONLINE' : 'DRIVE OFFLINE'}
                    </div>
                </div>
