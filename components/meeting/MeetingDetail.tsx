...
const MeetingDetail: React.FC<MeetingDetailProps> = ({
    ...
    const handleCopySummary = () => {
        // Simple and robust HTML parser that converts HTML rich-text into clean markdown/LINE formatted plain text.
        const htmlToPlainText = (html: string): string => {
            if (!html) return '';
            let text = html;

            // Replace line breaks & list items first with readable plain characters
            ...
            // Bold formatting for LINE (*boldText*)
        ...
        const mainContentText = htmlToPlainText(content);
        let extraSheetsText = '';
        if (sheets && sheets.length > 0) {
            extraSheetsText = sheets.map(s => {
                const plainContent = htmlToPlainText(s.content);
                return plainContent ? `\n\n📄 *${s.title}:*\n${plainContent}` : '';
            }).filter(Boolean).join('');
        }
...
💬 *บันทึก (Notes):*
${mainContentText}${extraSheetsText || ''}
...
_Generated by ${BRAND_CONFIG.name}_ 🍹
        `.trim();
